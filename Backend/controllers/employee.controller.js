...
exports.employee_confirm = function (req, res) {
    Employee.updateOne(
        ...
        function (err, employee) {
            if (err) return next(err);
            res.send("You Confirmed Date Successfully. !!");
        }
    );
}
...
exports.employee_cancelConfirm = function (req, res) {
    Employee.updateOne(
        ...
        {
            $set: {
                confirm: " Canceled Date",
                interviewSheduled: false
            }
        },
        function (err, employee) {
            if (err) return next(err);
            res.send("    You Canceled Date. If you want to shedul another date immediately contact which details in the message");
        }
    );
}
